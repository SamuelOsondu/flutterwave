...
from api.serializers import OrderSerializer, CreateOrderSerializer, OrderItemSerializer
from fondstore import settings
...
class OrderViewSet(ModelViewSet):
    ...
    @action(detail=False, methods=['GET'])
    def successful_orders(self, request):
        successful_orders = Order.objects.filter(status='success')
        serializer = OrderSerializer(successful_orders, many=True)
        return Response(serializer.data)

    @action(detail=False, methods=['GET'])
    def failed_orders(self, request):
